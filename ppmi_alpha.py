...
def main(args: argparse.Namespace) -> None:
    ...
    with open(args.results_path, "w") as sink:
        ...
        for (pair, cpair) in cpairs.items():  # type: ignore
            if self.args.smoothing is 0: 
                pxy = cpair / npairs
                (x, y) = pair
                px = cwords[x] / nwords
                py = cwords[y] / nwords
            else:
                pxy = (cpair+self.alpha) / (npairs*len(cpairs)*self.alpha)
                (x, y) = pair
                px = (cwords[x]+alpha) / (nwords*len(cwords)*alpha)
                py = (cwords[y]+alpha) / (nwords*len(cwords)*alpha)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--smoothing",
        default=0,
        help="flag to include add-alpha smoothing or not. (default: %(default)s)",
    )
    parser.add_argument(
        "--alpha",
        default=0.5,
        help="alpha value for add-alpha smoothing (default: %(default)s)",
    )
